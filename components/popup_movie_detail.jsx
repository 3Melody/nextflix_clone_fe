...
export default function MovieDetailModal({ isOpen, closeModal , movieDetail }) {
      ...
      <Dialog as="div" className="relative z-10" onClose={closeModal}>
       ...
       <div className="overflow-y-auto scrollbar-glass max-h-[95vh]">
                ...
                <div className="relative h-100 w-full">
                    <img src={movieDetail.posterUrl} alt={movieDetail.title} className="w-200 h-full object-cover rounded"></img>
                    <div className="absolute left-0 bottom-0 p-6 z-10 w-full flex items-center flex-col ">
                        <div className="text-white text-3xl mb-5 font-bold">{movieDetail.title}</div>
                        <div className="flex gap-4 items-center w-full ">
                            <button className="bg-white text-black py-2 md:px-8 w-full md:w-auto rounded"> 
                                <i className="fa-solid fa-play mr-2 ml-[-10px] md:ml-0"></i>Play</button>
                            <button className="border text-white w-10 h-10 rounded-full hidden md:flex items-center justify-center"> 
                                <i className="fa-solid fa-plus text-xl"></i></button>
                            <button className="border hidden text-white w-10 h-10 rounded-full md:flex items-center justify-center"> 
                                <i className="fa-solid fa-thumbs-up text-xl"></i></button>
                            <button className="border hidden text-white w-10 h-10 rounded-full md:flex items-center justify-center"> 
                                <i className="fa-solid fa-thumbs-down text-xl"></i></button>
                        </div>
                    </div>
                        ...
                        <div className="flex flex-col gap-2 text-sm text-gray-400">
                           ...
                           <div className="flex gap-4 md:hidden ">
                                <button className="border text-white w-10 h-10 rounded-full md:hidden flex items-center justify-center"> 
                                    <i className="fa-solid fa-plus text-sm"></i></button>
                                <button className="border md:hidden text-white w-10 h-10 rounded-full flex items-center justify-center"> 
                                    <i className="fa-solid fa-thumbs-up text-sm"></i></button>
                                <button className="border md:hidden text-white w-10 h-10 rounded-full flex items-center justify-center"> 
                                    <i className="fa-solid fa-thumbs-down text-sm"></i></button>
                           </div>
                    ...
                    <div>
                        <ul>
                            {episodeList.map((episode) => (
                                <li key={episode.id} className="flex items-center gap-4 my-4 cursor-pointer  hover:bg-white/10 p-2 rounded">
                                    <img src={movieDetail.posterUrl} alt={episode.title} className="w-32 h-20 object-cover rounded"></img>
                                    <div>
